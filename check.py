...
class MeterValidator:
    ...
    def validate_file(self, file_path, user_info, context=None):
        try:
            with db_transaction() as cursor:
                # Проверяем, есть ли отклоненные запросы "Убыло" за последние 5 дней
                cursor.execute('''
                    SELECT 1 FROM pending_requests 
                    WHERE user_tab = ? AND status = 'rejected'
                    AND timestamp > datetime('now', '-5 days')
                    AND inv_num IN (
                        SELECT inv_number FROM final_report 
                        WHERE name = ? AND location = ? AND division = ?
                        AND date > datetime('now', '-5 days')
                    )
                    LIMIT 1
                ''', (user_info['tab_number'], user_info['name'], user_info['location'], user_info['division']))
                
                if cursor.fetchone():
                    return {
                        'is_valid': False,
                        'errors': ["У вас есть отклоненные запросы 'Убыло'. Пожалуйста, отправьте показания заново без статуса 'Убыло'."],
                        'warnings': []
                    }
    ...
    def save_to_final_report(self, file_path_or_df, user_tab_number=None):
        ...
        try:
            ...
            # Сохраняем в базу данных с перезаписью существующих записей
            with db_transaction() as cursor:
                for _, row in df.iterrows():
