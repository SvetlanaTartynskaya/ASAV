...
def update_data_from_1c(context: CallbackContext = None):
    try:
        ...
        update_db_from_excel(users_df, shifts_df, equipment_df)
...
def handle_tab_number(update: Update, context: CallbackContext) -> int:
    try:
        ...
        with db_transaction() as cursor:
            cursor.execute('''
                SELECT tab_number FROM Users_user_bot WHERE chat_id = ?
                UNION
                SELECT tab_number FROM Users_admin_bot WHERE chat_id = ?
                UNION
                SELECT tab_number FROM Users_dir_bot WHERE chat_id = ?
            ''', (chat_id, chat_id, chat_id))
            existing_user = cursor.fetchone()
            
            if existing_user and existing_user[0] != tab_number:
                update.message.reply_text(
                    "❌ Этот Telegram-аккаунт уже привязан к другому табельному номеру. "
...
def handle_button(update: Update, context: CallbackContext):
    ...
    if text == 'В начало':
        ...
        return handle_tab_number(update, context)
...
def add_user_to_db(tab_number, name, role, chat_id, location, division):
    """Добавление пользователя в базу данных с проверкой chat_id"""
    try:
        # First check if chat_id is already registered
        with db_transaction() as cursor:
            cursor.execute('''
                SELECT tab_number FROM Users_user_bot WHERE chat_id = ?
                UNION
            ...
            if result and result[0] != tab_number:
                logger.warning(f"Chat ID {chat_id} already registered to another user")
                return False
...
def show_next_equipment(update: Update, context: CallbackContext, user_type='user'):
    ...
    try:
        ...
        # Создаем клавиатуру
        buttons = [
            [InlineKeyboardButton("В ремонте", callback_data=f'repair_{user_type}'), InlineKeyboardButton("Убыло", callback_data=f'ubylo_{user_type}')]
        ]
...
def finish_manual_input(update: Update, context: CallbackContext):
    try:
        # Проверяем, есть ли отклоненные запросы "Убыло"
        validator = MeterValidator()
        with db_transaction() as cursor:
            cursor.execute('''
        ...
        confirmation_message = "✅ Ваши показания:\n\n"
        for reading in readings:
            ...
            confirmation_message += (
                f"🔹 {equipment['Счётчик']} (Инв. № {equipment['Инв. №']}): "
                f"{value if value is not None else 'Нет показания'}"
            )
            if comment:
                confirmation_message += f" - {comment}"
            confirmation_message += "\n"
...
def handle_ubylo_rejection(update: Update, context: CallbackContext):
    """Отклонение запроса 'Убыло' с уведомлением пользователя и отменой сохранения отчета"""
    query = update.callback_query
    query.answer()
    ...
    try:
        with db_transaction() as cursor:
            ...
            cursor.execute('''
                ...
                WHERE name = ? AND location = ? AND division = ?
                AND date >= datetime('now', '-5 days')
            ''', (user_name, location, division))
            ...
            if os.path.exists(report_folder):
                for filename in os.listdir(report_folder):
                    if f"_{location}_{division}_{user_tab}_" in filename:
            ...
            query.edit_message_text(
                ...
                f"Счётчик: {meter_type}\n"
                f"Пользователь: {user_name}\n\n"
                f"Удалено {deleted_count} записей пользователя."
            )
...
def main():
    ...
    # Ежедневное обновление данных в 8:00 по Москве
    job_queue.run_daily(
        callback=daily_update,
        time=time(hour=8, minute=0, tzinfo=moscow_tz),
        days=(0, 1, 2, 3, 4, 5, 6)
    )
    
    job_queue.run_repeating(
        callback=update_data_from_1c,
        interval=4*3600,
        first=0
    )
    ...
    job_queue.run_daily(
        callback=cleanup_old_requests,
        time=time(hour=0, minute=0, tzinfo=moscow_tz),
        days=(0, 1, 2, 3, 4, 5, 6)
    )
    ...
    dp.add_handler(MessageHandler(Filters.regex('^Посмотреть показания за эту неделю$'), handle_view_week_report))
