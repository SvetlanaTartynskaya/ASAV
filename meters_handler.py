...
from time_utils import get_localized_time
...
def get_localized_time(location: str, dt: datetime = None) -> datetime:
    if dt is None:
        dt = datetime.utcnow()
    return dt.astimezone(get_user_timezone(location))

def format_local_time(location: str, dt: datetime = None) -> str:
    if dt is None:
        dt = get_localized_time(location)
...
def get_deadline_time(location: str) -> datetime:
    ...
    moscow_deadline = datetime.now(moscow_tz).replace(
...
def schedule_reminder(context: CallbackContext, tab_number: int, name: str, 
    ...
    try:
        ...
        if days_until_wednesday == 0 and now.hour >= hour:
            days_until_wednesday = 7
        
        reminder_time = (now + timedelta(days=days_until_wednesday))\
        ...
        context.job_queue.run_once(
            ...
            context={
                ...
                'deadline': format_local_time(location, get_deadline_time(location))
            },
            name=f"reminder_{tab_number}"
        )
        
        logger.info(f"Reminder for {name} scheduled for {reminder_time} ({user_tz.zone})")
    except Exception as e:
        logger.error(f"Error scheduling reminder for {tab_number}: {e}")
...
def send_reminder(context: CallbackContext):
    ...
    try:
        ...
        current_time = format_local_time(location)
        
        # Получаем временную зону для локации
        ...
        # Настраиваем дедлайн (14:00 по Москве)
        moscow_tz = pytz.timezone('Europe/Moscow')
        deadline_time = time(hour=14, minute=0)
        deadline_datetime = moscow_tz.localize(datetime.combine(get_localized_time(location).date(), deadline_time))
        ...
        message_text = (
            f"⏰ *Уважаемый {name}, необходимо подать показания счетчиков!*\n\n"
            f"📍 Локация: {location}\n"
        ...
        context.bot.send_document(
            chat_id=tab_number,
            document=InputFile(output, filename=f'Показания_{location}_{division}_{current_local_time.date()}.xlsx'),
        ...
        logger.error(f"Неизвестная временная зона для локации: {location}")
    except Exception as e:
        logger.error(f"Ошибка отправки напоминания {tab_number}: {e}")
        context.bot.send_message(
            chat_id=tab_number,
            text="⚠️ Произошла ошибка при подготовке напоминания. Пожалуйста, сообщите администратору."
        )
...
def handle_meters_file(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if tab_number:
            with db_transaction() as cursor:
                cursor.execute('''
                    ...
                    WHERE user_tab = ? AND status = 'rejected'
                    AND timestamp > datetime('now', '-5 days')
                    LIMIT 1
                ''', (tab_number,))
                if cursor.fetchone():
                    update.message.reply_text(
                        "❌ У вас есть отклоненные запросы 'Убыло'. "
                        "Пожалуйста, отправьте показания заново без статуса 'Убыло'."
                    )
                    return
        ...
        current_week = get_localized_time(location).strftime('%Y-W%U')
        report_folder = f'meter_readings/week_{current_week}'
        os.makedirs(report_folder, exist_ok=True)
        
        # Сохраняем файл
        timestamp = get_localized_time(location).strftime('%Y%m%d_%H%M%S')
        ...
        submission_time = datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')
        ...
        df['timestamp'] = submission_time

        is_on_time = check_if_on_time(location)
...
def check_if_on_time(location: str = None) -> bool:
    try:
        if not location:
            location = 'Москва'
        ...
        deadline_time = time(hour=14, minute=0)
        deadline_datetime = moscow_tz.localize(datetime.combine(get_localized_time(location).date(), deadline_time))
        local_deadline = deadline_datetime.astimezone(user_tz)
        
        return now < local_deadline and now.weekday() == 4
    except Exception as e:
...
def check_missing_reports(context: CallbackContext):
    ...
    try:
        ...
        current_week = get_localized_time(location).strftime('%Y-W%U')  # Год-Номер недели
        report_folder = f'meter_readings/week_{current_week}'
        os.makedirs(report_folder, exist_ok=True)
...
def notify_managers_about_unresolved_disagreements(context: CallbackContext):
    ...
    try:
        ...
        three_days_ago = (get_localized_time(location) - timedelta(days=3)).strftime('%Y-%m-%d %H:%M:%S')
